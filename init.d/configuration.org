...
**** PID
     ...
     #+begin_src emacs-lisp
       (cl-defstruct plt/ps
         ...
         (pid :documentation "Process ID" :type 'integer)
         (time :documentation "Time when the process was started" :type 'time)
         (ppid :documentation "Parent Process ID" :type 'integer)
         (user :documentation "User that started the process" :type 'string)
         (comm :documentation "The command that launched the process" :type 'string)
         (cmd :documentation "The command and all its args" :type string))
     #+end_src
