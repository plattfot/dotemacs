           ...
           (-each (--map (oref it :path) (tig-git-status))
             (lambda (note-file)
               (when-let ((message (plt/org-todo-commit--message
                 ...
                 (shell-command
                  (format "git add %S && git commit -m %S" note-file message)))))))
     #+end_src
**** Notmuch integration
...
**** Org-noter
     ...
     Allow to take notes in an org buffer when reading. One annoying
     issue with it right now is that it kills my frames everytime I
     quit taking notes. And this is with
     =org-noter-kill-frame-at-session-end= set to =nil=.
     ...
     #+begin_src emacs-lisp
       (use-package org-noter
         ...
         :after (org-roam)
         :config
         (setf org-noter-notes-window-location 'other-frame
