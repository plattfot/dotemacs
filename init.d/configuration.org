...
Setting =dabbrev-upcase-means-case-search= and =dabbrev-case-replace=
directly instead of relying on =case-fold-search= and =case-replace=
otherwise the suggestions for =dabbrev= looses their case, which makes
it unusable when programming.
...
#+begin_src emacs-lisp
  (setup yaml-mode
    (:hook plt-show-trailing-whitespace-if-writable))
#+end_src
...
#+begin_src emacs-lisp
  (setup detatched
    (detached-init)
    (:global [remap async-shell-command] detached-shell-command)
    (:option detached-shell-history-file "~/.histfile"
...
#+begin_src emacs-lisp
  (setup notmuch
    (:needs "notmuch" "msmtp")
    (:global "C-c e" notmuch
             "C-c E" plt-home-mail-refresh)
...
#+begin_src emacs-lisp
  ...
  (defun plt/start-irc ()
    "Connect to all irc networks that I use."
    (interactive)
    (erc-tls :server "irc.libera.chat"
             :port 6697
             :nick erc-nick
             :full-name erc-full-name
             :client-certificate
             `(,(expand-file-name "~/.ssl/libera.key")
               ,(expand-file-name "~/.ssl/libera.cer")))
    (erc-tls :server "irc.oftc.net"
             :port 6697
             :nick erc-nick
             :full-name erc-full-name
             :client-certificate
             `(,(expand-file-name "~/.ssl/oftc.key")
               ,(expand-file-name "~/.ssl/oftc.cer"))))
#+end_src
...
#+begin_src emacs-lisp
  (setup org
    ...
    (:hook plt-show-trailing-whitespace)
...
#+begin_src emacs-lisp
  ...
  (defun plt/org-todo-commit-change ()
    ...
    (when-let ((default-directory (car (project-roots (project-current nil)))))
      (-each (--map (oref it :path) (javel-git-status))
        (lambda (note-file)
...
#+begin_src emacs-lisp
  (setup citar
    ...
    (:option citar-library-paths `(,plt/paper-directory ,@plt/book-directories)
             citar-bibliography `(,plt/paper-bibtex ,plt/book-bibtex)
             org-cite-global-bibliography `(,plt/paper-bibtex ,plt/book-bibtex)
                 ...
                 `(,@(plt/elfeed-group-feeds
                   ...
                   ,@(plt/elfeed-group-feeds
                      ...
                      :feeds '("https://www.reddit.com/r/bapcsalescanada/.rss"
                               "https://www.reddit.com/r/MovieDealsCanada/.rss"))
                   ,@(plt/elfeed-group-feeds
                      :tags 'youtube
...
#+begin_src emacs-lisp
  ...
  (defvar plattfot-mode-map
    (let ((map (make-sparse-keymap)))
      ...
      (define-key map (kbd "C-c -") 'plt/insert-em-dash)
      (define-key map (kbd "C-c l") 'duplicate-line)
      (define-key map (kbd "C-c ,") 'split-at-comma)
      ;; Docs
      (define-key map (kbd "<f5>") 'shortdoc-display-group)
      ;; Buffer functions
      ...
      (define-key map (kbd "<f2>") 'plt-home-setup)
      map)
