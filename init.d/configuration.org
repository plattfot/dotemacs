...
*** Embark
    ...
    #+begin_src emacs-lisp
      ...
      on the file name and add the result to `kill-ring'."
        (let ((file (buffer-file-name (if (bufferp buffer)
                                          buffer
                                        (get-buffer buffer)))))
          (if file
              (kill-new (funcall file-transform file))
            (message "No filename copied, %s is not associated with a file."
                     buffer))))
