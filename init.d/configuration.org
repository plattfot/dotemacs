     ...
     #+begin_src emacs-lisp
       (use-package dabbrev
         ...
         :bind (("M-/" . dabbrev-completion)
                ("C-M-/" . dabbrev-expand))
         :config
         (setf dabbrev-upcase-means-case-search t
...
*** Yaml
    #+begin_src emacs-lisp
      (use-package yaml-mode
        :config
        (add-hook 'yaml-mode-hook #'dotemacs-show-trailing-whitespace-if-writable)
        :pin melpa-stable)
    #+end_src
*** json
    #+begin_src emacs-lisp
      (use-package json-mode
        :config
        (add-hook 'json-mode-hook #'dotemacs-show-trailing-whitespace-if-writable)
...
*** Ledger
   ...
   #+begin_src emacs-lisp
     (use-package ledger-mode
       ...
       (add-hook 'ledger-mode-hook (lambda () (setq-local orderless-smart-case nil)))
...
*** direnv
    ...
    #+end_src
*** dtache
    Allow to detach running commands from the shell. Using recommended
    settings from the [[https://gitlab.com/niklaseklund/dtache/-/tree/remote][readme]].
    ...
    #+begin_src emacs-lisp
      (use-package marginalia-dtache
        ...
        :config
        (add-to-list 'marginalia-annotators-light '(dtache . marginalia-dtache-annotate)))
    #+end_src
                  ...
                  (format "git add %S && git commit -m %S" note-file
                          (thread-last message
                            ...
                            (replace-regexp-in-string
                             (rx "[[" (one-or-more graph) "]["
                                 (group (one-or-more (any graph blank))) "]]") "\\1")
...
**** Org-roam
     ...
     #+begin_src emacs-lisp
       ...
       "
             :unnarrowed t)
            ("b" "band" plain #'org-roam-capture--get-point
             "
       ,#+begin_quote
       %? ---
       ,#+end_quote
       ...
       "
             :unnarrowed t)
            ("D" "datordel" plain #'org-roam-capture--get-point
             "
       ,* Senast observerade plats
       %? "
             :file-name "%<%Y%m%d%H%M%S>-${slug}"
             :head ":PROPERTIES:
       :URL: %(read-string \"Url: \")
     ...
     **NOTE:**
     ...
     #+begin_src emacs-lisp
       (use-package org-ref
         :ensure nil
         :config
         (setf wt/orcb-%-replacement-string orcb-%-replacement-string
    ...
    #+begin_src emacs-lisp
      (use-package mood-line
        ...
        (defun plt/mood-line-shell-modified (mood-line-fun)
          ...
          (if (not (derived-mode-p 'comint-mode 'vterm-mode))
